...
if($user_type == 'buyers'){
	$meta_key	= '_send_by';
	$label		= esc_html__('Owner name', 'taskbot');
}
			...
			<table class="table tb-table tb-dbholder">
				<thead>
					<tr>
						...
						<th><?php esc_html_e('Owner Name', 'taskbot');?></th>
				...
				<?php while ( $taskbot_query->have_posts() ) : $taskbot_query->the_post();
					...
					<tr>
						...
						<td data-label="<?php esc_html_e('Owner Name', 'taskbot');?>"><a href="<?php echo esc_url(get_the_permalink($seller_profile_id));?>"><?php echo esc_html(taskbot_get_username($seller_profile_id))?></a> </td>
