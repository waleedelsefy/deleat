...
$hide_filter_type               = !empty($taskbot_settings['hide_buyer_filter_type']) ? $taskbot_settings['hide_buyer_filter_type'] : false;
$hide_filter_location           = !empty($taskbot_settings['hide_buyer_filter_location']) ? $taskbot_settings['hide_buyer_filter_location'] : false;
$hide_filter_skills             = !empty($taskbot_settings['hide_buyer_filter_skills']) ? $taskbot_settings['hide_buyer_filter_skills'] : false;
$hide_filter_level              = !empty($taskbot_settings['hide_buyer_filter_level']) ? $taskbot_settings['hide_buyer_filter_level'] : false;
$hide_filter_languages          = !empty($taskbot_settings['hide_buyer_filter_language']) ? $taskbot_settings['hide_buyer_filter_language'] : false;
$hide_filter_price              = !empty($taskbot_settings['hide_buyer_filter_price']) ? $taskbot_settings['hide_buyer_filter_price'] : false;
$hide_buyer_without_avatar      = !empty($taskbot_settings['hide_buyer_without_avatar']) ? $taskbot_settings['hide_buyer_without_avatar'] : 'no';


$listing_type       = !empty($taskbot_settings['buyer_listing_type']) ? $taskbot_settings['buyer_listing_type'] : 'left';
...
$buyer_type        = !empty($_GET['buyer_type']) ? $_GET['buyer_type'] : array();
...
$buyer_location    = !empty($_GET['location']) ? esc_html($_GET['location']) : '';
...
/* Owner type */
if ( is_array($buyer_type) && !empty($buyer_type) ) {
    $tax_query_args[] = array(
        'taxonomy' => 'tb_buyer_type',
        'field'    => 'slug',
        'terms'    => $buyer_type,
        'operator' => 'IN',
    );
...
if ( !empty($buyer_location) ) {
    ...
    if( !empty($state) ){
        $countries_obj  = new WC_Countries();
        $countries      = $countries_obj->get_allowed_countries('countries');
        $states_list    = $countries_obj->get_states( $buyer_location );
...
if(!empty($hide_buyer_without_avatar) && $hide_buyer_without_avatar === 'yes'){
    $meta_query_args[] = array(
        'key'       => 'is_avatar',
        'value'     => 1,
        'compare'   => '=',
...
$buyer_data = new WP_Query(apply_filters('taskbot_freelancer_search_filter', $query_args));
$total_posts = $buyer_data->found_posts;
...
if( !empty($listing_type) && $listing_type === 'top' ){
    ...
    $grid_arg['buyer_type']        = $buyer_type;
    $grid_arg['buyer_data']        = $buyer_data;
    ...
    $grid_arg['buyer_location']    = $buyer_location;
    ...
    if(!empty($theme_version->get( 'TextDomain' )) && ( $theme_version->get( 'TextDomain' ) === 'taskup' || $theme_version->get( 'TextDomain' ) === 'taskup-child' )){
        get_template_part( 'template-parts/find', 'buyers', $grid_arg);
    }
} else {
?>
<main class="tb-main overflow-hidden tb-main-bg">
    <div class="tb-freelancersearch">
        <section class="hero"  style="width: 1519px; left: 0px;">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="heading">
                            <h3 class="text-center">Contact us</h3>
                            <h2 class="text-center">We will Be Happy to Help You</h2>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
                ...
                <div class="row gy-4">
                    ...
                    <div class="col-lg-4 col-xxl-3 tk-mt0">
                        <form id="tb_sort_form" class="tk-searchlist">
                            ...
                            <aside class="tb-sidebar">
                                ...
                                <?php
									if( !empty($hide_filter_type) ){ do_action('taskbot_render_buyer_type_filter_html',$buyer_type); }
									if( !empty($hide_filter_level) ){ do_action('taskbot_render_english_level_filter_html',$english_level);}
                                    if( !empty($hide_filter_location) ){ do_action('taskbot_location_search_field', $buyer_location);}
                    ...
                    <div class="col-lg-8 col-xxl-9">
                        <?php if ($buyer_data->have_posts()) {
                            while ($buyer_data->have_posts()) {
                                $buyer_data->the_post();
                                $buyer_id        = get_the_ID();
                                $buyer_name      = taskbot_get_username($buyer_id);
                                $tb_post_meta     = get_post_meta($buyer_id, 'tb_post_meta', true);
                                $buyer_tagline   = !empty($tb_post_meta['tagline']) ? $tb_post_meta['tagline'] : '';
                                        ...
                                        <div class="tb-bestservicedetail__user">
                                            <div class="tk-price-holder">
                                                <div class="tb-asideprostatus">
                                                    <?php do_action('taskbot_profile_image', $buyer_id,'',array('width' => 300, 'height' => 300));?>
                                                    <div class="tb-bestservicedetail__title">
                                                        <?php if( !empty($buyer_name) ){?>
                                                            <h6>

                                                                <a href="<?php echo esc_url( get_permalink()); ?>"><?php echo esc_html($buyer_name); ?></a>
                                                                <?php do_action( 'taskbot_verification_tag_html', $buyer_id ); ?>
                                                            </h6>
                                                        <?php } ?>
                                                        <?php if( !empty($buyer_tagline) ){?>
                                                            <h5><?php echo esc_html($buyer_tagline); ?></h5>
                                                        <?php } ?>
                                                        <ul class="tb-rateviews">
                                                            <?php do_action('taskbot_get_freelancer_rating_cuont', $buyer_id); ?>
                                                            <?php do_action('taskbot_get_freelancer_views', $buyer_id); ?>
                                                            <?php do_action('taskbot_save_freelancer_html', $current_user->ID, $buyer_id, '_saved_buyers', '', 'buyers'); ?>
                                                        </ul>
                                                    </div>
                                                </div>
                                                <?php do_action('taskbot_user_hourly_starting_rate', $buyer_id); ?>
                                            </div>
                                            <div class="tk-tags-holder">
                                                <?php the_excerpt(); ?>
                                                <?php do_action( 'taskbot_term_tags', $buyer_id,'skills','',15,$skills_base );?>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <?php if($total_posts > $per_page){?>
                            <?php taskbot_paginate($buyer_data); ?>
                        <?php }?>
                    </div>
