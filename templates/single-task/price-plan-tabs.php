...
$governorate	    = get_post_meta( $post_id, 'state', true );
...
<div class="tb-asideholder tb-sidebartabholder">
	<div class="tb-asidebox tb-sidebartabs">
		...
		foreach($plans as $key => $plan ){
			...
			$reported				= !empty($plan['reported']) ? $plan['reported'] : '';
			$rate				= !empty($plan['rate']) ? $plan['rate'] : '';
            $margin				= !empty($plan['margin']) ? $plan['margin'] : '';
            $location				= !empty($plan['_project_location']) ? $plan['_project_location'] : '';
			$description		= !empty($plan['description']) ? $plan['description'] : '';
			$price				= !empty($plan['price']) ? $plan['price'] : '';
			$established				= !empty($plan['established']) ? $plan['established'] : '';
			$acqui_perc				= !empty($plan['acqui_perc']) ? $plan['acqui_perc'] : '';
            $emply				= !empty($plan['emply']) ? $plan['emply'] : '';
			...
			if( !empty($title) && !empty($price) ){
				...
				$tab_contents	.='<a href="javascript:void(0);" data-url="'.esc_url( $cart_url ).'" data-type="task_cart" class="tb-btn '.esc_attr($checkout_class).'">'.esc_html__('Submit offer','taskbot').'<i class="icon-arrow-right"></i></a>';
				$tab_contents	.='</div>';
				$tab_contents	.='</div>';
				$tab_contents	.='</div>';
		...
		<div class="tab-content" id="tb_tasktakscontents">
			...
			<?php if( !empty($plans_count) && $plans_count>1){ ?>
				<div class="tb-share-section">
					<span class="tb-recommend"><?php esc_html_e('Compare Offers','taskbot');?><i class="icon-refresh-ccw"></i></span>
				</div>
			<?php } ?>
    ...
    <div class="tk-project-boxx">
        <ul class="tk-project-requirement">
            <li>
                ...
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                        <span><?php echo  $emply, ' ', 'Employee'; ?></span>
                    </div>
                    <em>Employees</em>
                </div>
            </li>
            <li>
                <i class="icon-pie-chart tk-yellow-icon"></i>
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                        <span><?php echo  $acqui_perc, ' ', '%'; ?></span>
                    </div>
                    <em>Acquisition Percentage</em>
                </div>
            </li>
            <li>
                <i class="icon-briefcase tk-darkred-icon"></i>
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                        <span><?php echo  $established, ' ', 'years'; ?></span>
                    </div>
                    <em>Established</em>
                </div>
            </li>
            <li>
                <i class="icon-bar-chart tk-yellow-icon"></i>
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                        <span><?php echo  $reported, ' ', ' L.E'; ?></span>
                    </div>
                    <em>Reported Sales in last 12 months</em>
                </div>
            </li>
            <li>
                <i class="icon-activity tk-yellow-icon"></i>
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                        <span><?php echo  $rate, ' ', ' L.E'; ?></span>
                    </div>
                    <em>Run Rate Sales</em>
                </div>
            </li>
            <li>
                <i class="icon-pie-chart tk-yellow-icon"></i>
            ...
            <li>
            <i class="icon-map-pin tk-yellow-icon"></i>
                <div class="tk-project-requirement_content">
                    <div class="tk-requirement-tags">
                <span>
                    <?php  echo WC()->countries->states[$country][$governorate]; //to get State name by state code ?>
                    ,
                    <?php  echo WC()->countries->countries[$country]; //to get State name by state code ?>
                </span>
                    </div>
                    <em>Location</em>
                </div>
