...
if ( $taskbot_query->have_posts() ) :
    ...
    while ( $taskbot_query->have_posts() ) : $taskbot_query->the_post();
        ...
        $auditor_id          = get_post_field( 'post_author', $post );
        $auditor_profile_id  = taskbot_get_linked_profile_id($auditor_id, '','auditors');
        $auditor_name        = taskbot_get_username($auditor_profile_id);
                ...
                <div class="tk-project-box">
                    ...
                    <div class="tk-employerproject">
                        <div class="tk-employerproject-title">
                            <?php do_action( 'taskbot_auditor_proposal_status_tag', $post->ID );?>
                        ...
                        <?php if( isset($project_price) ){?>
                            <div class="tk-price">
                                ...
                                <?php } elseif( !empty($post_status) && in_array($post_status,array('hired','cancelled','completed','disputed')) ){ ?>
                                    <div class="tk-project-detail">
                                        <a class="tk-btn-solid" href="<?php echo esc_url(Taskbot_Profile_Menu::taskbot_profile_menu_link('projects', $auditor_id, true, 'activity',$post->ID));?>"><?php esc_html_e('Project activity','taskbot');?></a>
                                    </div>
                                <?php } ?>
                            </div>
                        <?php } ?>
                                    ...
                                    <div class="modal-body tk-popup-content">
                                        ...
                                        <div class="tk-popup-info">
                                            <?php if( !empty($auditor_name) ){?>
                                                <h6><?php sprintf(esc_html__('Hi %s,','taskbot'),$auditor_name);?></h6>
                                            <?php } ?>
