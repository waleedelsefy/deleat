...
$order_url         			= Taskbot_Profile_Menu::taskbot_profile_menu_link('projects', $user_identity, true, 'activity',$proposal_id);

if($dispute_author_user_type == 'sellers'){
	$dispute_author_user_type	= esc_html__('Owner', 'taskbot');
	...
	<div class="row">
		<div class="col-sm-12 col-lg-8 col-md-12">
			<?php if(get_post_status($dispute_id) == 'publish' && $user_type == 'sellers'){?>
				<div class="tb-refunddetailswrap tb-refunddetailswrap-alert">
					...
					<?php foreach ($comments as $key => $value) {
						...
						if (!empty($user_roles) && is_array($user_roles) && in_array( 'administrator', $user_roles, true ) ) {
							...
							if($comment_user_type == 'sellers'){
								$comment_author_type	= esc_html__('Owner', 'taskbot');
					...
					<div class="tb-refunddetailttabs">
						<?php if($user_type == 'sellers' && (!in_array(get_post_status($dispute_id), array('resolved', 'disputed', 'refunded', 'cancelled')))){
							...
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								<li>
									<a class="nav-link tb-replytabbtn project-dispute-action active tb-btn" data-action="reply" data-submit_title="<?php esc_html_e('Post reply', 'taskbot');?>" href="javascript:void(0);"><?php esc_html_e('Reply to refund', 'taskbot');?></a>
								</li>
								<li>
									<a class="nav-link tb-allowtabbtn project-dispute-action tb-btn" data-action="refund" data-submit_title="<?php esc_html_e('Process Refund', 'taskbot');?>" href="javascript:void(0);"><?php esc_html_e('Allow refund', 'taskbot');?></a>
								</li>
								<li>
									<a class="nav-link tb-declinetabbtn project-dispute-action tb-btn" data-action="decline" data-submit_title="<?php esc_html_e('Decline refund', 'taskbot');?>" href="javascript:void(0);"><?php esc_html_e('Decline refund', 'taskbot');?></a>
								</li>
							</ul>
						<?php }?>
									...
									<form class="tb-themeform tb-refundform_form" id="dispute-reply-form">
										...
										<fieldset>
											...
											<div class="form-group tb-form-btn">
												<a href="javascript:void(0);" id="project-dispute-reply-btn" class="tb-btn tb-replytabbtn"><?php esc_html_e('Post reply', 'taskbot');?></a>
											</div>
										</fieldset>
									</form>
								</div>
			...
			<aside>
				<?php 
					...
					if( empty($proposal_type)  || $proposal_type === 'fixed' || $proposal_type === 'milestone' ){
						do_action('taskbot_proposal_order_budget_details', $proposal_id, $user_type);
					} else {
						do_action('taskbot_hourly_proposal_order_budget_details', $proposal_id, $user_type);
					}
				?>	
			</aside>
		</div>
