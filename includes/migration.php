...
if (!function_exists('taskbot_migration_auditors')) {
    ...
    function taskbot_migration_auditors() {
		$title_array			= array(
			'Brand strategy and digital marketing',
			'Sports and e-sports marketing scripting',
			'Direct inward dialing',
		...
		foreach ($freeauditors as $auditor_id) {
			$tb_post_meta     			= get_post_meta($auditor_id, 'tb_post_meta', true);
			$tb_post_meta['tagline']   = $title_array[array_rand($title_array)];
			update_post_meta($auditor_id, 'tb_post_meta', $tb_post_meta);
		}
	}
}
...
if (!function_exists('taskbot_user_location')) {
    ...
    function taskbot_user_location() {
		...
		$arg_user    = array(
            'fields'            => 'ids',
            'post_type'         => array('buyers','sellers','auditors'),
            'post_status'       => 'any',
            'numberposts'       => -1,
        );
...
if (!function_exists('taskbot_update_auditor_packages')) {
    ...
    function taskbot_update_auditor_packages($post_author) {
		global $woocommerce;
		$woocommerce->cart->empty_cart();
		$wallet_amount				= rand(5000,9500);
		$product_id                 = taskbot_buyer_wallet_create();
...
if (!function_exists('taskbot_migration_profile_data')) {
    ...
    function taskbot_migration_profile_data() {
        ...
        if( !empty($freeauditors) ){
            foreach($freeauditors as $profile_id){
			...
			foreach($employer_ids as $profile_id){
				...
				for ($i=0; $i < 1 ; $i++) {
					...
					if( !empty($project_id) ){
						...
						update_post_meta( $project_id, 'no_of_freeauditors',rand(1,5) );
