...
add_image_size('taskbot_thum_auditor_image', 200, 200, true);
add_image_size('taskbot_seller_image', 164, 164, true);
add_image_size('taskbot_auditor_image', 164, 164, true);
...
if (!function_exists('taskbot_get_linked_profile_id')) {
    function taskbot_get_linked_profile_id($id='', $type='users',$role='') {
		...
		if( $type === 'post') {
            ...
            } elseif (!empty($role) && $role === 'auditors') {
               $linked_profile = get_user_meta($id, '_linked_profile_auditor', true);
            }
		}
    ...
    function taskbot_get_user_role_type($user_identity) {
        ...
        if (!empty($user_identity)) {
			...
			if ( in_array( 'sellers', (array) $data->roles ) ) {
				...
				return 'buyers';
			} elseif ( in_array( 'auditors', (array) $data->roles ) ) {
				return 'auditors';
...
if ( ! function_exists( 'taskbot_dashboard_page_uri' ) ) {
    function taskbot_dashboard_page_uri( $user_type = '' ) {
        ...
        if ( !empty($redirect_type) && ($redirect_type === 'post_task') && ($user_type === 'sellers') ) {
            ...
            $redirect = !empty($_SESSION["redirect_url"]) ? $_SESSION["redirect_url"] : $redirect;
        } elseif ( !empty($redirect_type) && ($redirect_type === 'task_cart') && ($user_type === 'auditors') ) {
            $redirect = !empty($_SESSION["redirect_url"]) ? $_SESSION["redirect_url"] : $redirect;
...
if ( ! function_exists( 'taskbot_auth_redirect_page_uri' ) ) {
    function taskbot_auth_redirect_page_uri( $redirect_type = '', $user_id='' ) {
		...
		} elseif(  !empty($redirect_type) && ($redirect_type === 'login') && $user_type === 'auditors'){
            $login_redirect	= !empty($taskbot_settings['login_redirect_auditor']) ? $taskbot_settings['login_redirect_auditor'] : 'home';
	...
	function taskbot_get_user_avatar( $sizes = array(), $user_identity = '' ) {
		...
		if ( !empty( $thumb_id ) ) {
			...
			if( !empty($user_type) && $user_type == 'sellers') {
				...
				$default_avatar	= !empty($taskbot_settings['defaul_auditors_profile']) ? $taskbot_settings['defaul_auditors_profile'] : array();
			}
...
if ( ! function_exists( 'taskbot_service_list' ) ) {
    function taskbot_service_list( $type = '' ) {
		$list	= array(
			'1'	=> array(
				'title' 	=> esc_html__('Business introduction', 'taskbot'),
				'class'		=> 'tb-addservice-step'
			),
			'2'	=> array(
				'title' 	=> esc_html__('Financial info', 'taskbot'),
					...
					<div class="tb-budgetlist">
						...
						<ul class="tb-planslist tb-totalfee">
							<li>
								<a href="javascript:void(0);">
									<h6>
										<?php esc_html_e('Total business budget', 'taskbot');?>:&nbsp;
										<span>
											(<?php 
												if(function_exists('wmc_revert_price')){
       ...
       <div id="import-users" class="import-users custom-import" style="display:none;">
            <form method="post" action="<?php echo taskbot_prepare_final_url('file','import_users'); ?>"  enctype="multipart/form-data">
				...
				<h3 class="theme-name">
					...
					<lable for="upload-dummy-csv" class="button button-primary upload-dummy-csv"><?php esc_html_e('Choose file','taskbot');?></lable>
				</h3>
				<div class="user-actions">
...
if (!function_exists('taskbot_import_users')) {
	function  taskbot_import_users(){
		...
		if (function_exists('taskbot_migration_auditor_packages')) {
			taskbot_migration_auditor_packages();
		}
...
if (!function_exists('taskbot_generate_profile')) {
	function  taskbot_generate_profile(){
		...
		if(!empty($type) && $type == 'sellers'){
        ...
        }else if(!empty($type) && $type == 'auditors'){
			$auditor_post = array(
				'post_title'    => wp_strip_all_tags($display_name),
				'post_status'   => 'publish',
				'post_author'   =>  $user_id,
			...
			$auditors_id = wp_insert_post($auditor_post);
			update_user_meta($user_id, '_linked_profile_auditor', $auditors_id);
			update_post_meta($auditors_id, 'tb_post_meta', $tb_post_meta);
			update_post_meta($auditors_id, '_linked_profile', $user_id);
			update_post_meta($auditors_id, '_is_verified', 'yes');
		}
		...
		if (is_user_logged_in()) {
			...
			$user_type	= taskbot_get_user_type($current_user->ID);
				if( !empty($user_type) && $user_type === 'sellers' ){
					...
					$classes[] = 'tb-user-logged-auditor';
				}
		}else{
...
if (!function_exists('taskbot_search_list_type')) {
	function taskbot_search_list_type(){
		$list	= array(
			...
			'auditor_search_page'		=> esc_html__('Auditors','taskbot')
		);
