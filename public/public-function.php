...
/**
 * create dispute for auditor
 *
 */
if( !function_exists('taskbotAuditorCreateDispute') ){
    function taskbotAuditorCreateDispute($user_id=0,$request=array(),$type=''){
        global $taskbot_settings;
        $order_id           = !empty($request['order_id']) ? intval($request['order_id']):'';
        $task_id            = !empty($request['task_id']) ? intval($request['task_id']):'';
        $dispute_issue      = !empty($request['dispute_issue']) ? esc_html($request['dispute_issue']):'';
        ...
        update_post_meta( $dispute_id, '_send_type', $user_type);
        update_post_meta( $dispute_id, '_send_by', $user_id);
        update_post_meta( $dispute_id, '_auditor_id', $user_id);
        update_post_meta( $dispute_id, '_buyer_id', $buyer_id);
        update_post_meta( $dispute_id, '_dispute_key', $dispute_issue);
        update_post_meta( $dispute_id, '_dispute_order', $order_id);
        update_post_meta( $dispute_id, '_task_id', $task_id);
        ...
        if (class_exists('Taskbot_Email_helper')) {
            $buyer      = get_user_by( 'id', $buyer_id );
            $buyer_name = $buyer->display_name;
            /* getting auditor info */
            $auditor         = get_user_by( 'id', $user_id );
            ...
            $order_total  = !empty($order_total) ? ($order_total) : 0;
            $emailData = array();
            $emailData['auditor_name']      = $auditor_name;
            $emailData['buyer_name']        = $buyer_name;
            $emailData['task_name']         = $task_name;
            $emailData['task_link']         = $task_link;
            $emailData['order_id']          = $order_id;
            $emailData['order_amount']      = $order_total;
            $status_auditor_refund	= !empty( $taskbot_settings['email_admin_new_dispute'] ) ? $taskbot_settings['email_admin_new_dispute'] : '';
            if( !empty($status_auditor_refund) ){
                if (class_exists('TaskbotDisputeStatuses')) {
                    $email_helper = new TaskbotDisputeStatuses();
                    $email_helper->dispute_received_admin_email($emailData);
                }
...
if (!function_exists('taskbot_seller_email')) {
	function taskbot_seller_email(){
		$seller_email = array(
            /* Owner Email on Post Task */
		array(
			'id'      => 'divider_post_task_templates',
			'type'    => 'info',
			'title'   => esc_html__( 'List Your Business', 'taskbot' ),
			'style'   => 'info',
		),
		array(
			'id'       => 'email_post_task',
    ...
    /* Owner Email on Task Rejected */
		array(
			'id'      => 'divider_task_rejected_templates',
			'type'    => 'info',
			'title'   => esc_html__( 'Owner task rejected', 'taskbot' ),
			'style'   => 'info',
		),
		array(
    ...
    /* Owner Email on Order Complete request declined */
		array(
    ...
    /* Owner Email on Order Completed */
		array(
    ...
    /* Owner Email on Order Activity */
		array(
			'id'      => 'divider_email_order_activity_templates',
			'type'    => 'info',
			'title'   => esc_html__( 'Owner order activity', 'taskbot' ),
			'style'   => 'info',
		),
		array(
    ...
    /* Owner Email on Refund request */
		array(
			'id'      => 'divider_order_seller_refund_templates',
			'type'    => 'info',
			'title'   => esc_html__( 'Owner refund', 'taskbot' ),
			'style'   => 'info',
		),
		array(
    ...
    /* Owner Email on Dispute Resolved */
		array(
    ...
    /* Owner Email on Package */
		...
		array(
			...
			'type'      => 'textarea',
			'default'   => wp_kses( __( 'Thank you for purchasing the package “{{package_name}}” You can now post a business and get offers.', 'taskbot'),
			array(
				'a'	=> array(
				'href'  => array(),
				'title' => array()
		...
		/* Owner Email on Project invitation */
		array(
		...
		/* Owner Email on proposal decline */
		array(
		...
		/* Owner Email on milestone completed */
		array(
		...
		/* Owner Email on milestone decline */
		array(
		...
		/* Owner Email on project refund request */
		array(
