        ...
        global  $taskbot_settings;
            ...
            if( !empty($type) ){
                ...
                if(!empty($type) && $type == 'post_task'){
                    ...
                    $json['redirect']           = taskbot_get_page_uri('login');
                    $json['message']            = '';
                    $json['message_desc']       = esc_html__('You must login as a buyer to send a message to this owner.', 'taskbot');
                }
...
if (!function_exists('taskbot_send_message')) {
    function taskbot_send_message()
    {
        ...
        if( !empty($user_type) && $user_type === 'buyers' ){
            ...
            $json['type']           = 'error';
            $json['message_desc']   = esc_html__('We"re sorry, but only buyers can send a message to the owner.', 'taskbot');
            wp_send_json( $json );
        }
...
if (!function_exists('taskbot_save_profile_settings')) {
    function taskbot_save_profile_settings()
    {
        ...
        $list = array(
            ...
            'birthday'   	  => esc_html__('birthday is required', 'taskbot'),
            'country'   	  => esc_html__('Country is required', 'taskbot'),
            'zipcode'    	  => esc_html__('Zip code is required', 'taskbot'),
    //        'seller_type'     => esc_html__('Owner type is required', 'taskbot'),
      //      'english_level'   => esc_html__('English level is required', 'taskbot'),
        //    'hourly_rate'     => esc_html__('Funding required is required', 'taskbot'),
        );
        ...
        $birthday        = !empty( $profile_data['birthday'] ) ? sanitize_text_field( $profile_data['birthday'] ) : '';
       ...
       /* update seller type term */
        ...
        /* update seller type term */
        if(isset($auditor_type) && $auditor_type !=''){
            wp_set_object_terms( $profile_id, intval( $auditor_type ), 'tb_auditor_type' );
        }
        ...
        $tb_post_meta['$birthday']    = $birthday;
...
if (!function_exists('taskbot_get_terms_subcategories_dropdown')) {
    function taskbot_get_terms_subcategories_dropdown($parent_id, $term_ids=array())
    {
        if( !empty($parent_id) ){
            ...
            $taskbot_args   = array(
                'show_option_none'  => esc_html__('Business type', 'taskbot'),
                'show_count'    => 0,
                'hide_empty'    => 0,
            ...
            $child_categories        = '<label class="form-group-title">'.esc_html__('Business type:','taskbot').'</label><span class="tb-select">';
...
if (!function_exists('taskbot_task_search_get_terms_subcategories_dropdown')) {
    function taskbot_task_search_get_terms_subcategories_dropdown($parent_cat_slug, $terms_slug  = array(),$option='')
    {
        ...
        if( !empty($parent_id) ){
            if( !empty($option) && $option === 'title' ){
                $terms_html        = '<h6>'.esc_html__('Business type','taskbot').'</h6>';
            }
...
if (!function_exists('taskbot_auditor_submit_dispute')) {
    ...
    function taskbot_auditor_submit_dispute() {
        global $current_user,$post,$taskbot_settings;
        $json = array();
        if( function_exists('taskbot_is_demo_site') ) { 
            taskbot_is_demo_site();
        ...
        taskbotAuditorCreateDispute($current_user->ID,$data);
    }
    add_action('wp_ajax_taskbot_auditor_submit_dispute', 'taskbot_auditor_submit_dispute');
}
/**
 * Create dispute
 *
