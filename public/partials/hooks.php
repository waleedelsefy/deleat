...
if (!class_exists('Taskbot_Template_Functions')) {
        ...
        */
        public function taskbot_package_details($package = '',$show_btn=true) {
            ...
            if($package->get_type() == 'buyer_packages'){
                ...
                $btn_label  = esc_html__('Request now', 'taskbot');
                ...
                <div class="tb-pricing__content">
                    ...
                    <ul class="tb-pricinglist">
                        ...
                        <li>
                            <div class="tb-pricinglist__content">
                                <span><?php esc_html_e('Number of business to post', 'taskbot'); ?></span>
                                <span><?php echo wp_sprintf( _n( '%1$s business', '%1$s business', $number_tasks_allowed, 'taskbot' ), $number_tasks_allowed );?></span>
                            </div>
                        </li>
                        <li>
                            <div class="tb-pricinglist__content">
                                <span><?php esc_html_e('Featured Business', 'taskbot'); ?></span>
    ...
    /**
     * Owner hourly price starting from
     *
     * @throws error
     * @author Waleed Elsefy <waleedelsefy@gmail.com>
     * @return
            ...
            if (!empty($seller_id)) {
                ...
                if (!empty($tb_hourly_rate)) {
                    ...
                    <div class="tb-startingprice">
                        <i><?php esc_html_e('Invest from', 'taskbot'); ?></i>
                        <em>
    ...
    ?>
        <div class="tk-aside-holder">
            <div class="tb-sidebartitle">
                <h5><?php esc_html_e('Funding scope','taskbot');?></h5>
            </div>
...
if ( ! function_exists( 'taskbot_seller_status_filter' ) ) {
    ...
    function taskbot_seller_status_filter() {
        ...
        ?>
            <div class="tk-aside-holder">
                <div class="tb-sidebartitle">
                    <h5><i class="icon-minus"></i> <?php esc_html_e('Owner type','taskbot');?></h5>
                </div>
                <div class="tb-sidebarcontent">
 ...
 * @return
 */
if ( ! function_exists( 'taskbot_auditor_status_filter' ) ) {
    add_action( 'taskbot_auditor_status_filter', 'taskbot_auditor_status_filter');
    function taskbot_auditor_status_filter() {
        $online_auditor  = (isset($_GET['online_auditor'])  && $_GET['online_auditor']  == 'on' ? "checked" : "");
        $offline_auditor = (isset($_GET['offline_auditor']) && $_GET['offline_auditor'] == 'on' ? "checked" : "");
        ?>
        <div class="tk-aside-holder">
            <div class="tb-sidebartitle">
                <h5><i class="icon-minus"></i> <?php esc_html_e('Auditor type','taskbot');?></h5>
            </div>
            <div class="tb-sidebarcontent">
                <div class="tb-checkboxholder">
                    <div class="tb-checkbox">
                        <input name="online_auditor" id="onlineauditor" type="checkbox" <?php echo esc_attr($online_auditor) ?> >
                        <label for="onlineauditor"><span><?php esc_html_e('Online Auditor', 'taskbot');?></span></label>
                    </div>
                    <div class="tb-checkbox">
                        <input name="offline_auditor" id="offlineauditor" type="checkbox" <?php echo esc_attr($offline_auditor) ?>>
                        <label for="offlineauditor"><span><?php esc_html_e('Offline Auditor', 'taskbot');?></span></label>
                    </div>
                </div>
            </div>
        </div>
...
if (!function_exists('taskbot_task_author')) {
    ...
    function taskbot_task_author($user_id,$type='sellers')
    {
        ...
        $task_by    = !empty($type) && $type === 'sellers' ? esc_html__('Business from','taskbot') : esc_html__('Business by','taskbot');
        ...
        if( !empty($delivery_date) ){?>
            <div class="tb-tabitemextras">
                <div class="tb-tabitemextrasinfo">
                    ...
                    <div class="tb-taskinfo">
                        <span><?php esc_html_e('Business investment time','taskbot');?></span>
        ...
        if( !empty($data_created) ){?>
            <div class="tb-tabitemextras">
                <div class="tb-tabitemextrasinfo">
                    ...
                    <div class="tb-taskinfo">
                        <span><?php esc_html_e('Business offer date','taskbot');?></span>
...
if (!function_exists('taskbot_login_user_menu_details')) {
    function taskbot_login_user_menu_details()
    {
        ...
        if( current_user_can('administrator') || ( !empty($taskbot_user_role) && ($taskbot_user_role == 'sellers' || $taskbot_user_role == 'buyers' || $taskbot_user_role == 'auditors') )){
        ...
        if (class_exists('ACF') && function_exists('get_field')) {
            ...
            if (!empty($user_details) && $user_details == 'yes') {
                if (is_user_logged_in()) {
                    ...
                    $items      .= '<div class="tk-navbarbtn"><span data-type="post_task" id="tk_post_task" class="tk-btn-solid-lg">' . esc_html__('List Your Business', 'taskbot') . '<i class="icon icon-plus"></i></span></div>';
                }
            }
        }
        return $items;
...
if (!function_exists('taskbot_save_freelancer_html')) {
    function taskbot_save_freelancer_html($current_user_id = '', $auditor_id = '', $key = '', $type = '', $saved_type = '')
    {
        ...
        $saved_class        = !empty($saved_items) && in_array($auditor_id, $saved_items) ? 'bg-redheart' : 'bg-heart';
        $action             = !empty($saved_items) && in_array($auditor_id, $saved_items) ? '' : 'saved';
        ob_start();
        if (!empty($type) && $type == 'list') { ?>
            <li>
                <a href="javascript:void(0);" class="tb-heart" data-action="<?php echo esc_attr($action); ?>"
                   data-post_id="<?php echo intval($auditor_id); ?>" data-id="<?php echo intval($user_id); ?>"
...
if (!function_exists('taskbot_render_price_range_filter_html')) {
    function taskbot_render_price_range_filter_html($title = '', $min_price = 0, $max_price = 0){
        global $taskbot_settings;
        $min_search_price       = !empty($taskbot_settings['min_search_price']) ? $taskbot_settings['min_search_price'] : 1000000;
        $max_search_price       = !empty($taskbot_settings['max_search_price']) ? $taskbot_settings['max_search_price'] : 500000000;
...
if (!function_exists('taskbot_activity_chat_history')) {
    function taskbot_activity_chat_history($value = array(), $type = 'parent', $user_id = 0)
    {
        ...
        $auther_url         = !empty($author_user_type) && $author_user_type === 'sellers' ||'auditors' ? get_the_permalink($author_profile_id) : '#';
        ...
        if (!empty($_GET['key']) && !empty($_GET['verifyemail'])) {
            ...
            if (!empty($user_identity)) {
                ...
                if ($confirmation_key === $verify_key) {
                    ...
                    if (!empty($user_type) && ($user_type == 'sellers' || $user_type == 'buyers' || $user_type == 'auditors')) {
			...
			<tr>
				...
				<td>
					<select name="type" id="taskbot-type">
						<option value="buyers"><?php esc_html_e('Investor','taskbot');?></option>
						<option value="sellers"><?php esc_html_e('Owner','taskbot');?></option>
						<option value="auditors"><?php esc_html_e('Auditor','taskbot');?></option>
				   </select><br>
            ...
            if( !empty($roles) && in_array('subscriber',$roles)){
                ...
                if (!is_wp_error($post_id)) {
                    ...
                    if (!empty($user_type) && $user_type === 'buyers') {
                        ...
                        update_post_meta($post_id, 'tb_hourly_rate', '');
                        update_user_meta($user_id, '_linked_profile', $post_id);
                        update_user_meta($user_id, '_user_type', 'auditors');
...
if (!function_exists('taskbot_tooltip_array')) {
	function taskbot_tooltip_array($key=''){
		$list	= array(
            ...
            'featured_task'	    => esc_html__('Featured Business','taskbot'),
		);
...
if (!function_exists('taskbot_listing_task_html_v1')) {
    function taskbot_listing_task_html_v1($product_id=0)
    {
        if(!empty($product_id)){
            $product            = wc_get_product($product_id);
            $product_author_id  = get_post_field ('post_author', $product->get_id());
            $linked_profile_id  = taskbot_get_linked_profile_id($product_author_id, '','sellers','auditors');
            $user_name          = taskbot_get_username($linked_profile_id);
            ob_start();
            ?>
            ...
            ?>
                <div class="tb-sidebarcontent">
                    <div class="tb-sidebarinnertitle">
                        <h6><?php esc_html_e('Invest from:','taskbot');?></h6>
