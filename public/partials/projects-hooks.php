...
/**
 * Owner notification for project invitation
 *
 */
function taskbotSellerProjectInvitation($project_id = 0, $profile_id = 0)
{
    ...
    $auditor_profile_id                  = !empty($auditor_id) ? taskbot_get_linked_profile_id($auditor_id, '', 'auditors') : '';
    $notifyDetails                      = array();
    ...
    if (class_exists('Taskbot_Email_helper') && !empty($invitation_email_switch)) {
        ...
        $emailData['auditor_email']      = get_userdata($auditor_id)->user_email;
        $emailData['buyer_name']        = taskbot_get_username($buyer_profile_id);
        $emailData['seller_name']       = taskbot_get_username($seller_profile_id);
        $emailData['auditor_name']       = taskbot_get_username($auditor_profile_id);
...
if (!function_exists('taskbotSaveProjectData')) {
    function taskbotSaveProjectData($data = array(), $type = '')
    {
        ...
        $auditor_id                    = !empty($type) && $type === 'auditor' ? $data['user_id'] : get_current_user_id();
        $json                       = array();
        ...
        if (!empty($step_id) && $step_id == 2) {
            ...
            $required_fields    = array(
                'title'             => esc_html__('audition title is required', 'taskbot'),
            ...
            $json['message']        = esc_html__('Audition step1', 'taskbot');
            ...
            if (class_exists('Taskbot_Email_helper') && !empty($project_id)) {
                ...
                if (class_exists('TaskbotProjectCreation')) {
                    ...
                    $emailData['auditor_id']                 = $auditor_id; //auditor id
